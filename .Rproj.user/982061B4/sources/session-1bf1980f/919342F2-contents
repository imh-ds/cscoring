#' Calculate Composite Score Loadings
#' 
#' @description
#' Create composite score loadings of scales by specifying the indicators that
#' go into each respective composite variable. Refer to the help documents for
#' `?regression_loadings`, `?correlation_loadings`, and `?average_loadings` to 
#' see how the loadings are calculated for each weighting scheme.
#' 
...
#' correlation_loadings <- composite_loadings(data = grit,
#'                                            varlist = varlist,
#'                                            weight = "correlation")
#' 
#' # Calculate regression-weighted composite scores
#' regression_loadings <- composite_loadings(data = grit,
#'                                           varlist = varlist,
#'                                           weight = "regression")
#'                                    
#' # Calculate unweighted average composite scores
#' average_loadings <- composite_loadings(data = grit,
#'                                        varlist = varlist,
#'                                        weight = "average")
#' 
#' @export
composite_loadings <- function(data = .,
                               varlist,
                               weight = "correlation"){
  ...
  if(weight == "average"){
    ...
    return_results <- average_loadings(data,
                                       varlist)
  ...
  if(weight == "correlation"){
    ...
    return_results <- correlation_loadings(data,
                                           varlist)
  ...
  if(weight == "regression"){
    ...
    return_results <- regression_loadings(data,
                                          varlist)
